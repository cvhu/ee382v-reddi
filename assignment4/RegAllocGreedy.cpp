...
unsigned RAGreedy::tryAssign(LiveInterval &VirtReg,
    ...
    Order.rewind();
    // PhysReg keeps the register number for allocation.
    unsigned PhysReg;

    // MISSING: Iterate through all the physical registers in Order.
    //          If, according to 'Matrix', there is no interference 
    //          when assigning a physical register to the virtual 
    //          register, stop iterating.
    PhysReg = Order.next(); 
    bool mark = false;
    while (PhysReg){
        if (Matrix->checkInterference(VirtReg, PhysReg) ==  LiveRegMatrix::IK_Free){//OK
            //return PhysReg;
            mark = true;
            break;
        }
    ...
    // MISSING: If no registers were available for allocation 
    //          or if the physical register obtained is
    //          preferred according to the allocation
    //          order hint (use Order.isHint function),
    //          then return the physical register for 
    //          allocation or to point out that there is 
    //          no trivial allocation.
    if (mark == false)
        return 0;
    ...
    // Otherwise, the physical register is not preferred or none was found.
    ...
    if (unsigned Hint = MRI->getSimpleHint(VirtReg.reg))
        if (Order.isHint(Hint)) {
            ...
            // MISSING: If the virtual register 'VirtReg' can be assigned
            //          to the preferred physical register (given by 'Hint')
            //          by performing an interference eviction 
            //          of cost at most 1, then return the physical register.
            //          See 'evictInterference' function.
            EvictionCost cost(1);
            if (canEvictInterference(VirtReg, Hint, true, cost)){//IsHint
    ...
    DEBUG(dbgs() << PrintReg(PhysReg, TRI) << " is available at cost " << Cost
            << '\n');
    // MISSING:
    // Try choosing a physical register p' such that 
    // eviction costs are at most 'Cost'. If such a p' 
    // can be found, then use p' for allocation. Otherwise,
    // use the physical register found in previous steps.
    
    if (unsigned PhysReg_p = RAGreedy::tryEvict(VirtReg, Order,NewVRegs,Cost)){
        return PhysReg_p;
    }
    return PhysReg;
}
...
unsigned RAGreedy::tryEvict(LiveInterval &VirtReg,
    ...
    while (unsigned PhysReg = Order.next()) {
        ...
        // MISSING: if the eviction costs of PhysReg is at least
        //          the limit given by CostPerUseLimit,
        //          ignore this candidate physical register.
        ...
        // MISSING: If the cost of evicting the 
        //          interferences of assigning PhysReg to 
        //          VirtReg is at most BestCost, then this is 
        //          the best physical register seen so far. Otherwise,
        //          go to the next candidate. 
        //          Also, if it's a preferred register according to isHint,
        //          stop iterating over the physical registers and 
        //          attempt to allocate this register.
        if (Order.isHint(PhysReg)){
            BestPhys = PhysReg;
            break;
        }
        if (canEvictInterference(VirtReg, PhysReg, false, BestCost)){//"so far in the description". Do we need keep iterate even better one?Yilin
            BestPhys = PhysReg;
            //break;
            /*
				No, we return the best at the end.
                You keep track of the best register found so far, but don't stop.
                The only exception is when you find a preferred register, in which case
                you stop iterating (because all preferred register have the best possible eviction cost).
                -- Oswaldo.
			*/
        }

    }
    // After considering all physical registers.

    // MISSING: If no physical register could be found, return 0. 
    //          Otherwise, evict the interferences of the 
    //          best candidate, and return the physical register.
    if (!BestPhys){
        return 0;
    }
...
unsigned RAGreedy::selectOrSplit(LiveInterval &VirtReg,
    ...
    // MISSING: If possible, assign a free physical register to VirtReg.
    if (unsigned PhysReg = tryAssign(VirtReg, Order, NewVRegs)){//not 0
        return PhysReg;
    }
    ...
    // MISSING: If the virtual register is not at the splitting stage,
    //          try to find a suitable physical register to assign 
    //          by allowing evictions.
    ...
    // MISSING: If the virtual register is at a stage previous 
    //          to the splitting stage,
    //          set its stage to the splitting stage, and 
    //          add VirtReg to NewVRegs to perform splitting
    //          around this virtual register, and return 0.
    if (Stage < RS_Split){
