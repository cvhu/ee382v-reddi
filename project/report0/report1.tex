...
\begin{figure}[hbt!]
	...
	\label{figure:prime}
 \end{figure}
The example we use here was to calculate the 25000th prime number\cite{googleIO}. The overall algorithm of calculating the 25000th prime number was illustrated in Algorithm\ref{algorithm:prime}.
...
Running these two pieces of code on the same machine showed that the runtime of C++ code was 5x faster than the JavaScript code. To improve the performance of the JavaScript code, we performed a profiling on the JavaScript code to determine the runtime of each function.
...
\end{align}
Second, \eqref{eq:2} was applied to get the extract the profiling information of each function from the log file. 
\begin{align}
./tools/mac-tick-processor v8.log\label{eq:2}
\end{align}
The profiling information extracted in this example was in \ref{figure:prime_profiling}. 
\begin{figure}[hbt!]
    \centering
        \subfigure[]{
